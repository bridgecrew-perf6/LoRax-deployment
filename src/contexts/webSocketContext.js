...
const gateways = [
  process.env.REACT_APP_HARDWARE_GATEWAY_1,
  process.env.REACT_APP_HARDWARE_GATEWAY_2,
];
...
export function WebSocketProvider(props) {
  ...
  useEffect(() => {
    const newConnection = new WebSocket("wss://firerisk.herokuapp.com/ws");
...
function getValueWarning({ sensor, value }) {
  ...
  if (isDangerousValue(value))
    return `The ${sensorToFullName[sensor]} is unsafe.`;
  else return "";
}
